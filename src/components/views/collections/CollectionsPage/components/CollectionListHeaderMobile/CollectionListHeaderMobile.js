...
const CollectionListHeaderMobile = ({
  collections,
  ...
  const allCheckedState = checkedCollectionIds.length === collections.length;

  const toggleSelectAllCheckboxChecked = useCallback(() => {
    setCheckedCollectionIds((prevCheckedCollectionIds) => {
      if (prevCheckedCollectionIds.length === collections.length) {
        return [];
      }
  ...
  return (
    <>
      <div className="collections-header pa3-5">
        <div className="collections-header__create vertical-flex">
          <div className="mb3">
...
export default React.memo(CollectionListHeaderMobile, areEqual);
